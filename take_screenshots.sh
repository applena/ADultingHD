...
#
# Requires: ScreenshotTests target in ADultingHDUITests
#
...
CONFIG_FILE_TMP="/tmp/adultinghd_screenshot_config.json"
...
echo "=========================================="
echo "  ADultingHD App Store Screenshot Capture"
echo "=========================================="
...
for device_spec in "${DEVICES[@]}"; do
    ...
    for LANG in "${LANGUAGES[@]}"; do
        ...
        if xcodebuild test-without-building \
            ...
            -destination "platform=iOS Simulator,name=$DEVICE_NAME,OS=$DEVICE_OS" \
            -derivedDataPath "$DERIVED_DATA" \
            -only-testing:"ADultingHDUITests/ScreenshotTests/$TEST_METHOD" \
            CODE_SIGNING_ALLOWED=NO \
            -quiet 2>&1; then
