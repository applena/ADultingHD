...
PROJECT="$PROJECT_DIR/ADultingHD.xcodeproj"
SCHEME="ADultingHD macOS"
SCREENSHOTS_DIR="$PROJECT_DIR/screenshots"
DERIVED_DATA="$PROJECT_DIR/.build/DerivedData"
APP_PATH="$DERIVED_DATA/Build/Products/Debug/ADultingHD.app"
...
echo "=========================================="
echo "  ADultingHD macOS Screenshot Capture"
echo "=========================================="
...
click_at() {
    local x="$1" y="$2"
    osascript -e "
    tell application \"ADultingHD\" to activate
    tell application \"System Events\"
        tell process \"ADultingHD\"
...
capture_window() {
    local output_path="$1"
    osascript -e 'tell application "ADultingHD" to activate' 2>/dev/null
    sleep 0.5
    local wid
    wid=$(get_window_id)
    if [[ -n "$wid" ]]; then
...
capture_locale() {
    ...
    killall "ADultingHD" 2>/dev/null || true
    sleep 1
