...
- **Household play** — Shipped via CloudKit `CKShare`: a household owner invites others (even across Apple IDs) through native participant management. Recipients review the invitation before joining, add the home separately or merge shared chores from a home they own, and switch between multiple homes. Merge retains the original home and personal chores; household completion history stays scoped to its source. Accepted invites sync the shared task list, completions, and leaderboard. Tasks can carry a `defaultAssigneeId` so a household can divide chores by member, and the activity feed/leaderboard shows who did what and when
...
- **No paid-support or hosted-service business** — The source is open (MIT) and the
  binary ships through the App Store under ShadowPuppet, LLC. Building it yourself is
  fine; there is no service tier, support contract, or hosted offering to sell.
