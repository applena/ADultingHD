...
final class DataStore {
    ...
    private func seedOnboardingTasksWhileSerialized(
        ...
        let catalogTasks = recommendedTasks.compactMap { catalogTask -> HouseholdTask? in
            ...
            var task = catalogTask.toHouseholdTask()
            // The home screen is the final onboarding destination, so make
            // every selected catalog task immediately visible there. Its
            // normal recurrence remains intact after this first occurrence.
            task.scheduledOverrideDate = today
            return task
        }
