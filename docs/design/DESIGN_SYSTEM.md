...
The repository ships only artwork that is referenced by the app at runtime.
Exploratory concept boards and superseded onboarding exports are intentionally
not included in the open-source release. See
[`ASSET_LICENSES.md`](./ASSET_LICENSES.md) for provenance and licensing.
...
- Use the runtime illustrations as art-direction references. Crop variants should
  be exported per size class before adding them to the asset catalog.
...
The ImageGen prompt log and asset license record live in `docs/design/`. Runtime
artwork lives under `ADultingHD/App/Assets.xcassets/`:

- `Onboarding/WelcomeHeroV1` opens the first-run flow on the "what is your
  name?" screen with compact iPhone, iPad, and wide macOS variants.
- `Onboarding/SharedHouseholdV1` supports the join, invite-choice, and invite
  onboarding steps.
- `Onboarding/ManageableSuggestionsV1` supports the suggested-task step, and
  `Onboarding/XPStreaksV1` supports the rewards step.
